...
import { CheckCircle2, Circle, ChevronDown, ChevronUp, ExternalLink, Lock, ArrowRight } from 'lucide-react'
...
export default function OnboardingChecklist() {
        ...
        fetch('/api/dashboard/onboarding')
            ...
            .then(d => {
                if (d.data) {
                    ...
                    setNextAction(d.data.nextAction || '')
                }
            })
    ...
    // Première étape non complétée et non verrouillée → destination du bouton bannière
    const nextStep = steps.find(s => !s.done && !s.locked)
    const nextStepConfig = nextStep ? STEP_CONFIG[nextStep.key] : null
            ...
            {nextAction && nextStepConfig && (
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <ArrowRight size={14} style={{ color: '#60a5fa', flexShrink: 0 }} />
                        <span style={{ color: '#93c5fd', fontSize: 13, fontWeight: 500 }}>{nextAction}</span>
                    </div>
                    <Link
                        href={`/${locale}${nextStepConfig.href}`}
                        style={{
                    ...
                    >
                        {nextStepConfig.cta} <ExternalLink size={11} />
                    </Link>
                </div>
            )}
            ...
            <AnimatePresence>
                ...
                >
                    {steps.map((step, i) => {
                        ...
                        const isNext = !step.done && !step.locked && steps.slice(0, i).every(s => s.done || s.locked)
                        const isClickable = !step.done && !step.locked
                        ...
                        return (
                            ...
                            >
                                {/* Step header */}
                                <div
                                    onClick={() => isClickable && setExpandedKey(isExpanded ? null : step.key)}
                                    style={{
                                ...
                                >
                                    {step.done
                                        ...
                                        : step.locked
                                            ? <Lock size={16} style={{ color: '#334155', flexShrink: 0 }} />
                                            : <Circle size={20} style={{ color: isNext ? '#3b82f6' : '#334155', flexShrink: 0 }} />
                                    }
                                    <div style={{ flex: 1 }}>
                                        <div style={{
                                            color: step.done || step.locked ? '#94a3b8' : 'white',
