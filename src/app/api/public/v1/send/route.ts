...
// Client admin (bypass RLS) — lecture agent + écriture conversation/message
...
export async function POST(request: NextRequest) {
    ...
    const { agent_id, to, message, metadata } = body

    if (!agent_id || !to || !message) {
        return NextResponse.json({
            error: 'Missing required fields: agent_id, to, message',
            code: 'BAD_REQUEST'
        }, { status: 400 })
    }
    ...
    if (!rateCheck.allowed) {
        logApiUsage(supabaseAdmin, {
            ...
            endpoint: '/api/public/v1/send', method: 'POST', statusCode: 429,
    ...
    if (!result.success) {
        logApiUsage(supabaseAdmin, {
            apiKeyId: apiKey!.id, userId: userId!, agentId: agent_id,
            endpoint: '/api/public/v1/send', method: 'POST', statusCode: 500,
            requestBody: body, responseMs: Date.now() - startTime, ipAddress: ip
        })
        return NextResponse.json(
    ...
    // ── Sauvegarder en DB (conversation + message) ────────────────────────
    ...
    logApiUsage(supabaseAdmin, {
        apiKeyId: apiKey!.id, userId: userId!, agentId: agent_id,
        endpoint: '/api/public/v1/send', method: 'POST', statusCode: 200,
